...
from modelss.eval import eval_on_task
from modelss.dataset import Dataset, get_tokenizer
from modelss.model import TranHGAT
...
def train(model, train_set, optimizer, scheduler=None, batch_size=32):
    ...
    for i, batch in enumerate(iterator):
        # for monitoring
        _, xs, y, _, masks = batch # 这里的xs本质上是数据集中的一行内容，只不过是用tokenizer生成的， bao
        # 这里根据 xs构建HHG图
        ...
        # forward
        optimizer.zero_grad()
        logits, y, _ = model(xs, y, masks)  # 看样子是一批一批的进入的，但Token embedding 不是
...
def initialize_and_train(trainset, validset, testset, attr_num, args, run_tag):
    ...
    model = TranHGAT(attr_num, device, True, lm=args.lm, lm_path=args.lm_path)
    if device == 'cpu':
...
if __name__ == "__main__":
    ...
    parser.add_argument("--task", type=str, default="Beer")
