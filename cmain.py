...
from model import MatchingModel_RGCN, RGCNModel,process_data
...
model = MatchingModel_RGCN(RGCNModel(in_channels, out_channels,4), hidden_dim).to(device)
...
writer = SummaryWriter(log_dir=f'runs/RGCN/{dataset}_experiment')
...
for epoch in range(60):
    ...
    if f1 > best_f1:
        ...
        best_epoch = epoch
        # 保存模型
        torch.save(model.state_dict(), f'./checkpoints/RGCN/{dataset}/best_model.pt')
